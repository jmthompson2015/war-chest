...
QUnit.test("exploitation()", assert => {
  ...
  assert.equal(Node.exploitation(node3), 0.2);
  assert.equal(Node.exploitation(node4), 0.3);
});

QUnit.test("exploration()", assert => {
  // Run.
  const parent = Node.create({ state: 1, winCount: 0, playoutCount: 10 });
  const node1 = Node.create({ state: 1, parent, winCount: 0, playoutCount: 0 });
  const node2 = Node.create({ state: 2, parent, winCount: 1, playoutCount: 0 });
  const node3 = Node.create({ state: 3, parent, winCount: 2, playoutCount: 5 });
  const node4 = Node.create({ state: 4, parent, winCount: 4, playoutCount: 10 });

  // Verify.
  assert.equal(Node.exploration(node1), Number.POSITIVE_INFINITY);
  assert.equal(Node.exploration(node2), Number.POSITIVE_INFINITY);
  assert.equal(round4(Node.exploration(node3)), 1.357);
  assert.equal(round4(Node.exploration(node4)), 0.96);
});
